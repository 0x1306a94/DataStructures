...
internal extension BinaryTreeNodeable {
    func dotStyle() -> String {
        return ""
    }

    func dotStyle() -> String where Extra == RBColor {
        guard self.extra == .red else {
            return "style=filled fillcolor=\"black\" color=\"black\" fontcolor=\"white\""
        }
        return "style=filled fillcolor=\"red\" color=\"red\" fontcolor=\"white\""
    }
}
...
internal extension BinaryTreeable {
    ...
    func generateDot() -> String where Self: ITree {
        ...
        if root.left != nil || root.right != nil {
            ...
            dotArray.append("\t\"\(val)\" [group=\"\(val)\", label=\"\(val)\" \(root.dotStyle())]")
        }
...
//                    out.append("\t\"\(leftVal)\" [group=\"\(leftVal)\", label=\"\(leftVal)\" \(root.left!.dotStyle())]")
//                }
...
//                    out.append("\t\"\(rightVal)\" [group=\"\(rightVal)\", label=\"\(rightVal)\" \(root.right!.dotStyle())]")
//                }
    ...
    func generateDot() -> String where Self: ITree, Self.Node.Extra == RBColor {
        guard let root = self.root else { return "" }
        var dotArray: [String] = [
            "digraph G {{",
            "\tgraph [nodesep=0.1]",
